...
사이트 안 업로드 창(드래그 앤 드롭)을 그대로 쓰고, 저장은 종훈님 **구글 드라이브**로 됩니다. 새로 가입할 곳 없이 지금 쓰시는 구글 계정 그대로 씁니다. 하객은 이 사이트 안에서만 업로드하고 어디로도 이동하지 않습니다.

연결에는 `apps-script/GuestSnapUpload.gs` 파일의 코드가 필요합니다 (구글의 "Apps Script"라는 무료 자동화 도구용 코드). 설정 방법:

1. [script.google.com](https://script.google.com) 접속 → **새 프로젝트**
2. 기본으로 있는 코드를 지우고 `apps-script/GuestSnapUpload.gs` 파일 내용을 전체 복사해서 붙여넣기
3. 우측 상단 **배포 → 새 배포** → 유형에서 **웹 앱** 선택
4. 실행 계정: **나**, 액세스 권한이 있는 사용자: **모든 사용자**로 설정 → **배포**
5. 구글 드라이브 접근 권한 승인 창이 뜨면 허용
6. 발급된 **웹 앱 URL**을 복사해서 저에게 알려주시거나, `script.js`의 `CONFIG.guestSnapUploadEndpoint`에 직접 붙여넣기

이렇게 하면 하객이 사진을 올릴 때마다 구글 드라이브에 "청첩장 게스트 스냅"이라는 폴더가 자동으로 생기고, 그 안에 사진·영상 파일과 함께 보낸 사람 성함·메시지가 정리된 스프레드시트("게스트 스냅 목록")도 같이 쌓입니다. 다운로드하실 땐 그 폴더를 통째로 우클릭 → 다운로드하면 됩니다. 용량은 구글 드라이브 기본 저장공간(무료 15GB)을 씁니다.

업로드 버튼은 결혼식 당일 0시부터 자동으로 열리고, 그 전에는 눌러도 파일 선택창이 안 뜹니다. 미리 테스트해보고 싶으면 주소 끝에 `?preview=1`을 붙여서 접속하면 날짜와 상관없이 항상 열려있습니다 (본인만 쓰는 테스트용 링크이니 하객에게 공유하는 링크와는 다릅니다).
